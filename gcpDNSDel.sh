...
# Remove the GCP TXT record
gcloud beta dns --account=${service_account_email} --project=${project} record-sets transaction start --zone=${zone_name} &> /dev/null
gcloud beta dns --account=${service_account_email} --project=${project} record-sets transaction remove $CERTBOT_VALIDATION --name=_acme-challenge.${domain}. --ttl=60 --type=TXT --zone=${zone_name} &> /dev/null
